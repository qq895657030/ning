 ...
 */
 const blackboxDeltaFieldDefinition_t blackboxMainFields[] = {
    ...
    // {"test_free",  -1, UNSIGNED, .Ipredict = PREDICT(0),       .Iencode = ENCODING(UNSIGNED_VB), .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(TAG8_8SVB), CONDITION(TEST_FREE)},
};
...
static void writeIntraframe(void)
{
    ...
    // blackboxWriteUnsignedVB(blackboxCurrent->test_free);
...
static void loadMainState(timeUs_t currentTimeUs)
{
    ...
    for (int i = 0; i < DEBUG16_VALUE_COUNT; i++) {
        // blackboxCurrent->debug[i] = debug[i];
            blackboxCurrent->debug[i] = i*1000;
    }

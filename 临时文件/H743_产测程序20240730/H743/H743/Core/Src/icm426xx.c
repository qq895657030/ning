...
//		}
    ...
    icm42688_write_reg(i,ICM42688_REG_BANK_SEL, 0); //设置bank 0区域寄存器
    icm42688_write_reg(i,ICM42688_REG_BANK_SEL, 0x01); //软复位传感器
    ICM42688DelayMs(100);

    if(Imu_ID[i] == ICM42688_ID)
    {
        icm42688_write_reg(i,ICM42688_REG_BANK_SEL, 1); //设置bank 1区域寄存器
        icm42688_write_reg(i,ICM42688_INTF_CONFIG4, 0x02); //设置为4线SPI通信
        ...
        icm42688_write_reg(i,ICM42688_REG_BANK_SEL, 0x00);
        reg_val = icm42688_read_reg(i,ICM42688_PWR_MGMT0); //读取PWR—MGMT0当前寄存器的值(page72)
        reg_val &= ~(1 << 5);//使能温度测量
